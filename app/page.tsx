  ...
  return (
    <main dir="rtl" className={dark ? "app dark" : "app"}>
      ...
      <section className="workspace">
        ...
        <div className="page">
          {view !== "dashboard" && <button className="globalBackButton" onClick={() => setView("dashboard")}><ArrowLeft size={17}/> رجوع للوحة التحكم</button>}
...
type SaleRow = { id:string; source:string; service_name:string|null; item_description:string; customer_name:string; customer_phone:string|null; customer_type?:"NEW"|"RETURNING"; quantity:number; total_amount:number; cost_amount:number; paid_amount:number; status:"COMPLETED"|"PENDING"|"CANCELLED"; notes:string|null; sold_at:string; created_by_name:string|null };
const emptySale = () => ({ customerName:"",customerPhone:"",customerType:"NEW" as "NEW"|"RETURNING",serviceName:"",quantity:1,unitPrice:0,unitCost:0,paidAmount:0,status:"COMPLETED" as SaleRow["status"],notes:"",soldAt:todayInCairo() });
function Sales({flash,services}:{flash:(message:string)=>void;services:ServiceRecord[]}){
  ...
  return <>
    ...
    <section className="panel salesEditor"><div className="panelHead"><div><h3>{editing?"تعديل المبيعة":"تسجيل مبيعة يدوية"}</h3><p>لا يشترط سحب حساب لتسجيل البيع.</p></div></div><div className="salesForm">
      <label>اسم العميل<input list="knownCustomers" value={form.customerName} onChange={e=>setForm({...form,customerName:e.target.value})}/><datalist id="knownCustomers">{Array.from(new Set(sales.map(s=>s.customer_name))).map(name=><option key={name} value={name}/>)}</datalist></label><label>نوع العميل<select value={form.customerType} onChange={e=>setForm({...form,customerType:e.target.value as "NEW"|"RETURNING"})}><option value="NEW">عميل جديد</option><option value="RETURNING">عميل حالي</option></select></label><label>الهاتف<input dir="ltr" inputMode="tel" value={form.customerPhone} onChange={e=>setForm({...form,customerPhone:e.target.value})}/></label><label>الخدمة<select value={form.serviceName} onChange={e=>selectService(e.target.value)}><option value="">اختر الخدمة</option>{services.filter(service=>service.active).map(service=><option key={service.id} value={service.name}>{service.name}</option>)}</select></label>
      <label>الكمية<input type="number" min={1} value={form.quantity} onChange={e=>setForm({...form,quantity:Math.max(1,Number(e.target.value)||1)})}/></label><label>سعر الوحدة<input type="number" min={0} value={form.unitPrice||""} onChange={e=>setForm({...form,unitPrice:Math.max(0,Number(e.target.value)||0),paidAmount:0})}/></label><label>تكلفة الوحدة<input type="number" min={0} value={form.unitCost||""} onChange={e=>setForm({...form,unitCost:Math.max(0,Number(e.target.value)||0)})}/></label><label>المدفوع<input type="number" min={0} max={formTotal} value={form.paidAmount||""} onChange={e=>setForm({...form,paidAmount:Math.min(formTotal,Math.max(0,Number(e.target.value)||0))})}/></label>
      <label>التاريخ<input type="date" value={form.soldAt} onChange={e=>setForm({...form,soldAt:e.target.value})}/></label><label>حالة العملية<select value={form.status} onChange={e=>setForm({...form,status:e.target.value as typeof form.status})}><option value="COMPLETED">مكتملة</option><option value="PENDING">معلقة</option><option value="CANCELLED">ملغاة</option></select></label><label className="wide">ملاحظات جاهزة<select value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}><option value="">بدون ملاحظات</option><option>ضيف دعوة</option><option>حساب مشترك</option><option>تجديد اشتراك</option><option>عرض خاص</option><option>دفعة أولى</option><option>متابعة لاحقة</option></select></label>
      <div className="saleCalculation wide"><div><span>إجمالي البيع</span><b>{formTotal.toLocaleString("ar-EG")} ج.م</b></div><div><span>إجمالي التكلفة</span><b>{formCost.toLocaleString("ar-EG")} ج.م</b></div><div><span>المتبقي</span><b>{formRemaining.toLocaleString("ar-EG")} ج.م</b></div><div className={formProfit<0?"loss":"profit"}><span>مجمل الربح</span><b>{formProfit.toLocaleString("ar-EG")} ج.م</b></div></div>
