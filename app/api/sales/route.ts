...
export async function PATCH(request:Request){
  const context=await requireWorkspacePermission("sales.manage");if(!context)return NextResponse.json({error:"FORBIDDEN"},{status:403});const id=new URL(request.url).searchParams.get("id");const parsed=saleSchema.safeParse(await request.json());if(!id||!parsed.success)return NextResponse.json({error:"INVALID_INPUT"},{status:400});const v=parsed.data;
  await query("ALTER TABLE sales ADD COLUMN IF NOT EXISTS customer_type TEXT NOT NULL DEFAULT 'NEW'");
