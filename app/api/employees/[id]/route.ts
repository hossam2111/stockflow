...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const user=await query("SELECT id,email,name,role,team,active,daily_limit FROM users WHERE id=$1 AND organization_id=$2",[id,context.organizationId]);
...
const schema=z.object({active:z.boolean(),dailyLimit:z.number().int().min(0).max(10000),permissions:z.array(z.object({serviceId:z.string(),enabled:z.boolean(),dailyLimit:z.number().int().min(0).max(10000)}))});
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  await transaction(async client=>{
    const updated=await client.query("UPDATE users SET active=$2,daily_limit=$3 WHERE id=$1 AND role='EMPLOYEE' AND organization_id=$4 RETURNING id",[id,parsed.data.active,parsed.data.dailyLimit,context.organizationId]);
