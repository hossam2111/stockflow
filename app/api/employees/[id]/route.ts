...
export async function GET(_request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const context=await getWorkspaceContext();if(!context?.organizationId)return NextResponse.json({error:"UNAUTHORIZED"},{status:401});if(context.session.role!=="ADMIN"&&context.session.id!==id)return NextResponse.json({error:"FORBIDDEN"},{status:403});
  const user=await query("SELECT id,email,name,role,team,active,daily_limit,can_manage_accounting FROM users WHERE id=$1 AND organization_id=$2",[id,context.organizationId]);
...
const schema=z.object({active:z.boolean(),dailyLimit:z.number().int().min(0).max(10000),canManageAccounting:z.boolean().default(false),permissions:z.array(z.object({serviceId:z.string(),enabled:z.boolean(),dailyLimit:z.number().int().min(0).max(10000)}))});
export async function PUT(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  await transaction(async client=>{
    const updated=await client.query("UPDATE users SET active=$2,daily_limit=$3,can_manage_accounting=$4 WHERE id=$1 AND role='EMPLOYEE' AND organization_id=$5 RETURNING id",[id,parsed.data.active,parsed.data.dailyLimit,parsed.data.canManageAccounting,context.organizationId]);
