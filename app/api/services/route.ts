...
export async function GET() {
  ...
  const result = await query(`SELECT s.id,s.name,s.active,s.default_daily_limit,s.default_cost,s.requires_otp,
    ...
    WHERE s.organization_id=$1 GROUP BY s.id,s.name,s.active,s.default_daily_limit,s.default_cost,s.requires_otp,s.created_at ORDER BY s.created_at`,[context.organizationId]);
...
export async function POST(request: Request) {
  ...
  const { name, defaultDailyLimit, defaultCost, requiresOtp } = parsed.data;
  ...
  for (const orgId of targets) {
    ...
    const result = await query<{ id: string; name: string; active: boolean; default_daily_limit: number; default_cost: number }>(
      `INSERT INTO services(id,organization_id,name,default_daily_limit,default_cost,requires_otp) VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT DO NOTHING
       RETURNING id,name,active,default_daily_limit,default_cost,requires_otp`, [`svc-${crypto.randomUUID().slice(0, 8)}`,orgId,name,defaultDailyLimit,defaultCost,requiresOtp]);
